...
export default function Home() {
  ...
  const loadChecklists = async () => {
    ...
    else setDashboardExecutions((executionRows || []) as DashboardExecution[]);
...
function Dashboard({checklists,executions,history,nonConformities,loading,error,setSection}:DashboardProps){
 ...
 <article className="panel recent"><div className="panel-head"><div><h2>Conclusões recentes</h2><p>Histórico persistido no Supabase</p></div><button className="text-link" onClick={()=>setSection("Histórico")}>Ver histórico</button></div>{history.length===0?<p className="data-state">Nenhuma execução concluída.</p>:history.slice(0,5).map(record=><div className="recent-row" key={record.id}><span className="recent-icon"><Icon name="calendar"/></span><span><strong>{record.checklist_name}</strong><small>{record.executor_name} · {new Date(record.completed_at).toLocaleString("pt-BR")}</small></span><span className="status">{record.conformity_percentage??0}%</span><Icon name="check" size={16}/></div>)}</article></section></>}</>
}
