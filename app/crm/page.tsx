...
export default function CrmPage() {
  ...
  const metrics = useMemo(()=>{
    ...
    return { researched, hot:prospects.filter(p=>p.tier==="HOT").length, contacted, demos, conversion:prospects.length?Math.round(won/prospects.length*100):0, revenue };
  },[prospects]);
  function addProspect(event:React.FormEvent) { event.preventDefault(); const next=enrichProspect(form); if (!next.company_name) return setNotice("Informe o nome da empresa."); setProspects(current=>[next,...current]); setSelected(next.id); setForm(empty); setNotice(`${next.company_name} entrou como ${next.tier} (${next.score}/100).`); }
  ...
  return <main className="sdr-shell">
    ...
    <section className="sdr-workspace">
      <div className="sdr-main">
        ...
        {importResult && <div className="sdr-import-result"><strong>Dry-run concluído</strong><span>{importResult.imported.length} novos · {importResult.conflicts.length} conflitos</span><button onClick={applyImport}>Importar somente não duplicados</button>{importResult.conflicts.map(conflict=><small key={`${conflict.row}-${conflict.reason}`}>Linha {conflict.row}: {conflict.reason}{conflict.duplicate?` — ${conflict.duplicate}`:""}</small>)}</div>}
        <div className="sdr-table"><div className="sdr-row sdr-head"><span>Empresa</span><span>Score</span><span>Etapa</span><span>Cidade</span></div>{visible.length?visible.map(p=><button className={`sdr-row ${selected===p.id?"selected":""}`} key={p.id} onClick={()=>setSelected(p.id)}><span><b>{p.company_name}</b><small>{p.segmento || "Sem segmento"}</small></span><span><i className={`sdr-tier ${p.tier.toLowerCase()}`}>{p.tier}</i> {p.score}</span><span>{p.stage}</span><span>{p.cidade || "—"}</span></button>):<p className="sdr-empty">Nenhum prospect nesta etapa.</p>}</div>
      </div>
      <aside className="sdr-detail">{selectedProspect ? <>
        <div className="sdr-detail-head"><div><p className="sdr-eyebrow">PROSPECT</p><h2>{selectedProspect.company_name}</h2><span>{selectedProspect.cidade || "Cidade não informada"} · {selectedProspect.segmento || "Segmento não informado"}</span></div><b className={`sdr-score ${selectedProspect.tier.toLowerCase()}`}>{selectedProspect.score}<small>/100</small></b></div>
