    ...
    {!loading&&checklist&&<div className="detail-layout">
      <section>
        ...
        <article className="detail-card">
          ...
          {sections.map(section=><div className="checklist-section" key={section.id}><h3>{section.title}</h3>{section.checklist_items.length===0&&<p className="empty-state">Nenhum item nesta seção.</p>}{section.checklist_items.map(item=><div className="checklist-item" key={item.id}><span className="item-check">✓</span><span><strong>{item.prompt}</strong><small>{item.required?"Obrigatório":"Opcional"} · {item.answer_type}</small></span></div>)}{canManage&&<div className="inline-create"><input value={itemPrompt[section.id]||""} onChange={event=>setItemPrompt(current=>({...current,[section.id]:event.target.value}))} placeholder="Novo item obrigatório"/><button className="secondary" onClick={()=>addItem(section.id)}>Adicionar item</button></div>}</div>)}
